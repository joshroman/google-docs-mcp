...
const { values } = parseArgs({
  ...
  options: {
    'gauth-file': { type: 'string', default: path.join(projectRoot, '.gauth.json') },
    'accounts-file': { type: 'string', default: path.join(projectRoot, '.accounts.json') },
    'credentials-dir': { type: 'string', default: projectRoot }
  }
});
...
let gauth: GAuthService;

// --- Initialization ---
async function initializeGAuthService() {
  gauth = new GAuthService(config);
  await gauth.initialize();
  console.error('GAuthService initialized with accounts:', gauth.getAccountList().map(a => a.email).join(', '));
}

// --- Helper to get clients for a specific user ---
async function getClientsForUser(userId: string): Promise<{
  docs: docs_v1.Docs;
  drive: drive_v3.Drive;
  sheets: sheets_v4.Sheets;
}> {
  const client = await gauth.ensureAuthorized(userId);
  return {
...
// --- Account Management Tool (no user_id required) ---

server.addTool({
  name: 'list_accounts',
  description: 'Lists all configured Google accounts that can be used with the Google Docs, Sheets, and Drive tools. Use this to find available user_id values. This tool does not require a user_id parameter.',
  parameters: z.object({}),
  execute: async (args, { log }) => {
    log.info('Listing configured Google accounts');

    const accounts = gauth.getAccountList();

    if (accounts.length === 0) {
      return 'No Google accounts configured. Please add accounts to .accounts.json';
    }

    let result = `Found ${accounts.length} configured Google account(s):\n\n`;
    accounts.forEach((account, index) => {
      result += `${index + 1}. **${account.email}**\n`;
      result += `   Type: ${account.accountType}\n`;
      if (account.extraInfo) {
        result += `   Info: ${account.extraInfo}\n`;
      }
      result += `\n`;
    });

    result += `\nUse one of these email addresses as the 'user_id' parameter in other tools.`;
...
parameters: UserIdParameter.merge(DocumentIdParameter).extend({
...
execute: async (args, { log }) => {
const docs = await getDocsClient(args.user_id);
...
parameters: UserIdParameter.merge(ApplyTextStyleToolParameters),
execute: async (args, { log }) => {
const docs = await getDocsClient(args.user_id);
...
parameters: UserIdParameter.merge(ApplyParagraphStyleToolParameters),
execute: async (args, { log }) => {
const docs = await getDocsClient(args.user_id);
...
execute: async (args, { log }) => {
const docs = await getDocsClient(args.user_id);
const drive = await getDriveClient(args.user_id);
...
server.addTool({
  ...
  parameters: UserIdParameter.merge(DocumentIdParameter),
  execute: async (args, { log }) => {
...
server.addTool({
  ...
  execute: async (args, { log }) => {
    ...
    try {
      ...
      const docsClient = await getDocsClient(args.user_id);
      const drive = await getDriveClient(args.user_id);
...
parameters: UserIdParameter.extend({
...
execute: async (args, { log }) => {
  const sheets = await getSheetsClient(args.user_id);
